    ...
    func saveNote(){
        if let note = note{
            do{
                ...
                try ream.write(){
                    if(note.title != self.titleTextField.text || note.content != self.contentTextView.text ) //note ในฐานข้อมูลเดิม มันไม่ตรงกับที่อยู่ใน textfield คือมันเกิดการเปลี่ยนแปลงขึ้น
                    {
